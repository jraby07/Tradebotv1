...
class TradeBot:
    def __init__(self, config: dict, simulate: bool = True):
        ...
        self.last_row = None
        self.balance = config.get('starting_balance', 10000.0)
        self.position = 0.0
    ...
    def calculate_position_size(self, price: float) -> float:
        pct = self.config.get('max_trade_percentage', 10) / 100.0
        usd_amount = self.balance * pct
        return usd_amount / price

    def check_exit_conditions(self, price: float) -> str | None:
        if self.position <= 0:
            return None
        rm = self.config.get('risk_management', {})
        stop_loss = self.entry_price * (1 - rm.get('stop_loss_percentage', 0) / 100)
        take_profit = self.entry_price * (1 + rm.get('take_profit_percentage', 0) / 100)
        if price <= stop_loss:
            return 'stop_loss'
    ...
    def run(self):
        ...
        while True:
            ...
            exit_reason = self.check_exit_conditions(last_price)
            if exit_reason:
                self.execute_trade('sell', self.position, last_price, exit_reason)
                self.position = 0.0
            if signal == 'buy' and self.position == 0:
                amount = self.calculate_position_size(last_price)
                self.execute_trade('buy', amount, last_price, reason)
                self.position = amount
                self.entry_price = last_price
            elif signal == 'sell' and self.position > 0:
                self.execute_trade('sell', self.position, last_price, reason)
                self.position = 0.0
